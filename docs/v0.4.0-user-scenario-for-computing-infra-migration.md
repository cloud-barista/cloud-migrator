...
> 이번 통합 테스트 및 릴리스를 위하여 사용자 시나리오를 개선이 필요합니다. 🙏
...
: Computing environment includes computing infrastructure, software, and data
: Currently, we're concentrating on the computing infrastructure.
...
> [수정] Web에서 /import/infra api 호출 전 /refresh api 호출 절차 추가했음.
...
## Retrieve the information of the source computing environment (refined data, on-premise model)
...
> [수정] Honeybee 제거 / Damselfly 조회 api 변경
...
> [수정] Damselfly 조회 api 수정 / provider & region 호출을 모델 선택 이후로 변경
