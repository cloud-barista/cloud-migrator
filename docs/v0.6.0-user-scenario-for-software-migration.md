...
sequenceDiagram
    ...
    %% Step 2: Access infra management and query available nodes
    User->>Browser: Access infar management menu in portal
    activate Browser
    Browser->>Butterfly: Request infra management view
    activate Butterfly

    %% Query infra list from Beetle
    Butterfly->>Beetle: API call to GET /migration/ns/{nsId}/infra (query infra list, or infra ID list with query param (option=id))
    activate Beetle
    Beetle-->>Butterfly: Return list of infras or infra IDs
    Butterfly-->>Browser: Respond with the list of infras or infra IDs
    Browser-->>User: Display the list of infras or infra IDs

    %% Query available infra from Beetle
    User->>Browser: Select an infra and query to get available nodes
    Browser->>Butterfly: Request the selected infra info to get available nodes
    Butterfly->>Beetle: API call to GET /migration/ns/{nsId}/infra/{infraId} (query available nodes)
    Beetle-->>Butterfly: Return list of available nodes and their details
    deactivate Beetle

    Butterfly-->>Browser: Respond with node list and details
    deactivate Butterfly
    Browser-->>User: Display available nodes for migration target
    deactivate Browser
    ...
    Butterfly->>Cicada: API call to GET /cicada/workflow_template (query software migration workflow templates)
    activate Cicada
    ...
    User->>Browser: Select target infrastructure and workflow template, create migration workflow
    activate Browser
    Browser->>Butterfly: Send workflow creation request with target infrastructure and template
    activate Butterfly
    Butterfly->>Cicada: API call to POST /cicada/workflow (create migration workflow based on target software model, target infrastructure, and selected template)
    activate Cicada
    Note over Cicada: Create software migration workflow:<br/>- Use target software model data<br/>- Apply selected workflow template<br/>- Configure target node information<br/>- Set task dependencies and execution order<br/>- Generate workflow DAG
    ...
    Grasshopper->>Tumblebug: Get target node connection info
    activate Tumblebug
    Tumblebug-->>Grasshopper: Return target node access info
